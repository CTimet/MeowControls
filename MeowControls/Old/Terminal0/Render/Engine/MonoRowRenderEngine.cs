...
) : IRowRenderEngine
{
    public void Render(RenderRow row, Point origin, DrawingContext context)
    {
        ...
        {
            if (GetBackground(row, i).Equals(basicBackground)) continue;

            //颜色发生了改变。把前面的颜色渲染出来
            context.DrawRectangle(basicBackground, new Pen(),
                new Rect(drawBackgroundFrom, new Size(i * cellWidth - origin.Y, cellHeight)));
            //然后改变基础背景色
        ...
        //我们最后还得把行末尾的颜色渲染出来
        context.DrawRectangle(basicBackground, new Pen(),
            new Rect(drawBackgroundFrom, new Size(row.Content.Length * cellWidth, cellHeight)));
        ...
        for (var i = 1; i < row.Content.Length; i++)
        {
            if (GetForeground(row, i).Equals(basicForeground)) continue;
            ...
            //渲染字符
            context.DrawText(new string(content).ToFormattedText(typeface, fontSize, basicForeground),
                drawForegroundFrom);
            //然后改变基础颜色
